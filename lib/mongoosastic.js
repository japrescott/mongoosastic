...
function hydrate(res, model, options, cb) {
	...
	Object.keys(hydrateOptions)
		.forEach(function getOpts(option) {
...
function Mongoosastic(schema, pluginOpts) {
	...
	var bulkTimeout, bulkBuffer = [],
		...
		parentKey = (options && options.parentKey) || false;
	...
	schema.methods.index = function schemaIndex(inOpts, inCb) {
		var index, type, serialModel, inst,
			cb = inCb,
			opts = inOpts;
		...
		if (bulk) {
			...
			if (parentKey && this[parentKey]) {
				inst.parent = this[parentKey].toString();
			}
			esClient.index(inst, cb);
		}
	};
	...
	schema.statics.search = function search(inQuery, inOpts, inCb) {
		...
		Object.keys(opts)
			.forEach(function collectKeys(opt) {
