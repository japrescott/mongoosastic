...
function getMapping (schema, options) {
	console.log("GetMapping..", options);
	let retMapping = {}
	generator.generateMapping(schema, options, (err, mapping) => {
		if (err) {
			// no handling needed
		}
...
function createMappingIfNotPresent (options, cb) {
	...
	console.log("createaMappingIfNotPresent..", schema, options);

	generator.generateMapping(schema, options, (ignoredErr, mapping) => {
		...
		console.log("Mapping for", indexName, "is ",completeMapping);

		client.indices.exists({
			index: indexName
		}, (err, exists) => {
			if (err) {
				console.log("mapping index errored", err);
				return cb(err)
			}

			if (exists) {
				console.log("mapping index exists, putMapping!", completeMapping);
			...
			console.log("create new indice..", settings);
			return client.indices.create({
				index: indexName,
				body: settings
			}, indexErr => {
				if (indexErr) {
					console.log("create new indice errored..", indexErr);
					return cb(indexErr)
				}

				console.log("mapping index created, putMapping!", completeMapping);
...
function Mongoosastic (schema, pluginOpts) {
	...
	let mapping = getMapping(schema, options)
	...
	let parentKey = (options && options.parentKey) || false
	let forceIndexRefresh = !(options && options.forceIndexRefresh === false)

	let joins = (options && options.joins) || false
	...
	function bulkDelete(opts, cb) {
		...
		if (parentKey && opts.model[parentKey]) {
			inst.delete.parent = opts.model[parentKey].toString()
		}
		bulkAdd(inst);
		cb();
	}
	...
	function bulkIndex(opts) {
		...
		if (parentKey && opts.model[parentKey]) {
			inst.index.parent = opts.model[parentKey].toString()

			// console.log("mongoosastic: parentKey",parentKey," is ", inst.index.parent);
		}

		if (joins) {
			inst.index[joins.field] = {
				name: joins.self
			}
			if (joins.parentField){
				inst.index[joins.field].parent = this[joins.parentField].toString()
			}
		}

		console.log("bulkIndex", inst, opts.model);
	...
	schema.methods.index = function schemaIndex (inOpts, inCb) {
		...
		if (bulk) {
			...
			console.log('Indexing bulk', _opts);
			...
			if (parentKey && this[parentKey]) {
				_opts.parent = this[parentKey].toString();
			}

			if (joins) {
				_opts.body[joins.name] = {
					name: joins.self
				}
				if (joins.parentField){
					_opts.body[joins.name].parent = this[joins.parentField].toString()
				}
			}

			console.log('Indexing not bulk', _opts);
	...
	schema.methods.unIndex = function unIndex (inOpts, inCb) {
		...
		if (parentKey && opts.model[parentKey]) {
			opts.parent = opts.model[parentKey].toString()
		}
	...
	schema.statics.esSearch = function (inQuery, inOpts, inCb) {
		...
		console.log('Searching for', JSON.stringify(esQuery, null, 4) );
