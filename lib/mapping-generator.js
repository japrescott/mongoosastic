...
function getMapping (cleanTree, inPrefix) {
  ...
  for (field in cleanTree) {
    ...
    // If indexing a string, and no es_type specified, use text as string is deprecated since ES5.x
    ...
    for (prop in value) {
      ...
      if (value.hasOwnProperty(prop) && prop.indexOf('es_') === 0 && !['es_indexed', 'es_join_name', 'es_join_isParentLink'].includes(prop)) {
...
function getJoinMapping(options){
  const mappingJoins = {};
  if (options && options.joins){
    mappingJoins[options.joins.name] = {
      type: 'join',
      relations: options.joins.relations,
    };
  }
  return mappingJoins;
}
...
Generator.prototype.generateMapping = function generateMapping (schema, options, cb) {
  ...
  //console.log("Generate mapping..", schema, options);
  ...
  let mappingJoin = getJoinMapping(options)
